...
suite_list, test_list, tag_list, kw_list, kw_times = [], [], [], [], []
...
def generate_report(opts):
    ...
    if opts.metrics_report_name:
        result_file_name = opts.path+opts.metrics_report_name
    else:
        result_file_name = 'AutomationReport.html'
    ...
    # Custom codes for tag
    logging.info(" Capturing tag metrics ")
    result.visit(TagResults(tag_list))

    if opts.showkeyword == "True":
        logging.info(" 3 of 4: Capturing keyword metrics")
        result.visit(KeywordResults(kw_list, IGNORE_LIBRARIES, IGNORE_TYPES))
        hide_keyword_menu = ""
    else:
        logging.info(" 3 of 4: Ignoring keyword metrics")
        result.visit(KeywordResults([], IGNORE_LIBRARIES, IGNORE_TYPES))
    ...
    # Custom code
    if tag_list:
        tag_stats,tagdf = dashboard_obj.get_tag_statistics(tag_list)
    else:
        tag_stats, tagdf = {},{}
